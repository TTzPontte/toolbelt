...
const NavItemLink = ({name, link, fa = "", active = ""}) => {
    ...
    const ThirdPartyLink = ({name, fa, link}) => (<a
            className="nav-link"
            href={link}
            target="_blank"
    ...
    const InternalLink = ({link, name, fa}) => (<Link className="nav-link " to={link}>
            <Info {...{name, fa}} />
        </Link>);
    return (<li className={`nav-item  ${active}`}>
            <Row className="w-100">
                <Col>
                    {!fa && <i className={fa}/>}
                    {isthirdPartyLink ? (<ThirdPartyLink {...{name, fa, link}} />) : (
...
const navlinks = [{
    links: [{
        link: "/orgs/home", name: "Home", fa: "home"
    }]
}, {
    title: "Simulação", links: [{
        link: "/orgs/simulation/HE", name: "Simulação H.E.", fa: "landmark", clsx: "my-0"
    }, {
        link: "/orgs/simulation/FI", name: "Simulação Finan.", fa: "hand-holding-usd"
    }]
}, {
    title: "Cadastro", links: [{
        link: "/orgs/leads/HomeEquity", name: "Home Equity", fa: "landmark", clsx: "my-0"
    }, {
        link: "/orgs/leads/Financing", name: "Financiamento", fa: "hand-holding-usd"
    }]
}, {
    title: "Biblioteca", links: [{
        link: "/orgs/blog/material-apoio", name: "Materiais de Apoio", fa: "building", clsx: "my-0"
    }, {
        link: "https://app.pipefy.com/request-tracker/", name: "Status das operações", fa: "address-card"
    }]
}];
...
const Menu = () => {
    ...
    if (pathParameters) {
        navlinks.push({
            title: "Usuário", links: [{
                link: "/orgs/users/invite", name: "Convite", fa: "user-plus"
            }]
        });
    }
    ...
    Menu.Section = ({title, links}) => (<>
            ...
            {links.map((i) => (<NavItemLink {...i} />))}
            <Divider/>
        </>);

    return (<>
            {navlinks.map((navlink) => (<Menu.Section {...navlink} />))}
        </>);
};
...
const FixedSidebar = ({toogled, setState, fixed, offCanvas}) => {
    ...
    return (<>
            <ul
                className={`navbar-nav bg-gradient-primary sidebar sidebar-light accordion ${toogled ? "toggled " : ""}`}
                id="accordionSidebar"
            >
                <a
...
const OffcanvasSidebar = ({openned = false}) => {
    return (<>
            <NavProvider state={openned}>
                {({show, setShow}) => (<>
                        {show && (<Offcanvas className={"navbar-nav bg-gradient-primary sidebar sidebar-dark accordion"}
...
export const Sidebar = (props) => {
    return (<>
            <section style={{paddingBottom: "1.5em"}}>
                <NavProvider>
                    {({
                          show, state, state: {fixed, offCanvas, toogled}, setState
                      }) => (<>
