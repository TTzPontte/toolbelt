...
const getAssociatedPartnerReports = (serasaReportId) => {
  return from(DataStore.observe(SerasaPartnerReport)).pipe(
      filter((report) => report.serasareportID === serasaReportId),
      mergeMap((filteredReports) => {
        if (filteredReports.length === 0) {
          return throwError(new Error('No associated SerasaPartnerReports found.'));
        }
...
// Usage:
getAssociatedPartnerReports('yourSerasaReportId').subscribe({
  next: (report) => {
    // Handle the retrieved SerasaPartnerReport here
    console.log('Retrieved SerasaPartnerReport:', report);
  },
  error: (error) => {
    // Handle errors here
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching data:", error);
        // You may want to handle or display this error to the user.
      }
    };
