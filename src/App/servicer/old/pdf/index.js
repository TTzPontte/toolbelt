...
function makeDd() {
  ...
  // Create Identification Table
  const identificationTable = createIdentificationTable(
    registration,
    tableGenerator
  );
  ...
  const negativeDataTables = createNegativeDataTable(
    negativeData,
    tableGenerator
  );

  // Create Optional Features Table
  const optionalFeaturesTable = createOptionalFeaturesTable(
  ...
  return {
    ...
    content: [
      ...
      scoreTable, // Add Negative Data Tables (pefin, refin, notary, check)
      ...negativeDataTables, // Add Optional Features Table
...
export function generatePDF() {
  const dd = makeDd();
  return pdfMake.createPdf(dd).download("Report.pdf");
}

// todo  write a generate pdf function that will generate a pdf using pdfmake and will be called form a react component
// const dd = makeDd();
