...
const createTable = (data, title, itemKeys, headers) => {
    ...
    if (data[itemKey] !== undefined) {
        const auxTable = data[itemKey].map(item => itemKeys.map(key => key(item)));
...
const createNotaryTable = (data, title) => createTable(data, title, [
    ...
    item => `${title} [${item?.federalUnit} - ${formatCurrency(item?.amount || 0)} - ${formatDateResume(item?.occurrenceDate || 0)}]`
], ["OfficeNumber", "Valor", "Data", "Cidade", "Estado", "Resumo"]);

const createNegativeTable = (data, title) => createTable(data, title, [
    ...
    item => item?.city,
    item => item?.federalUnit,
    item => `${title} [${item?.legalNature} - ${formatCurrency(item?.amount || 0)} - ${formatDateResume(item?.occurrenceDate || 0)}]`
...
const generateReportContentPF = (report, optional) => {
    ...
    return [
        ...
        tableGenerator.createInfoTable(
            ...
            [[registration.name, formatDocumentNumber(registration.documentId), registration.birthDate, registration.statusRegistration, registration.address.city, registration.address.state]]
        ),
        { style: "contentPDF", text: "\nDados de Score" },
        {
            columns: [
