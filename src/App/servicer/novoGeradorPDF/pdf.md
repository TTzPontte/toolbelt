...
First Module (helpers.js):
Imports various modules, including diacritics, and defines some styles for PDF content.
Defines two classes: TableFactory and TableGenerator.
TableFactory is used for creating tables with specific styles.
TableGenerator contains methods for generating different types of tables (score, summary, info, etc.) with specific data.
Defines utility functions for formatting document numbers, removing accents, formatting dates and currency, and converting values to percentages.
Exports the classes, styles, and utility functions.
Second Module (index.js):
Imports necessary dependencies like pdfMake, fonts, and the first module (helpers.js).
Defines two functions: generateReportContentPJ and generateReportContentPF.
generateReportContentPJ generates report content for a legal entity (PJ).
generateReportContentPF generates report content for an individual (PF).
Exports functions to generate PDF reports for both legal entities (PJ) and individuals (PF).
generateDDPJ and generateDDPF generate the report content and structure.
createPDF creates a PDF document and downloads it.
