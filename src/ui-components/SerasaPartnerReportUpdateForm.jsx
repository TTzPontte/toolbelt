...
export default function SerasaPartnerReportUpdateForm(props) {
  const {
    id: idProp,
    serasaPartnerReport: serasaPartnerReportModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = serasaPartnerReportRecord
      ? { ...initialValues, ...serasaPartnerReportRecord, SerasaReport }
      : initialValues;
    ...
    setSerasaReport(cleanValues.SerasaReport);
    setCurrentSerasaReportValue(undefined);
    setCurrentSerasaReportDisplayValue("");
    setErrors({});
  };
  const [serasaPartnerReportRecord, setSerasaPartnerReportRecord] =
    React.useState(serasaPartnerReportModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SerasaPartnerReport, idProp)
        : serasaPartnerReportModelProp;
      setSerasaPartnerReportRecord(record);
      const SerasaReportRecord = record ? await record.SerasaReport : undefined;
      setSerasaReport(SerasaReportRecord);
    };
    queryData();
  }, [idProp, serasaPartnerReportModelProp]);
  React.useEffect(resetStateValues, [serasaPartnerReportRecord, SerasaReport]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SerasaPartnerReport.copyOf(serasaPartnerReportRecord, (updated) => {
              Object.assign(updated, modelFields);
              if (!modelFields.SerasaReport) {
                updated.serasareportID = undefined;
              }
            })
          );
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "SerasaPartnerReportUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || serasaPartnerReportModelProp)}
